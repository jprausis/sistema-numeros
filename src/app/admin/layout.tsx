...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
        ...
        const fetchUser = async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    setRole(data.user?.role || 'INSTALLER');
                } else {
                    // Se falhar a API, tentamos o fallback do Supabase mas sabemos que pode ser falho
                    const { data: { user } } = await supabase.auth.getUser();
                    if (user) setRole(user.user_metadata?.role || 'INSTALLER');
                }
            } catch (error) {
    ...
    const renderIcon = (iconName: string) => {
        ...
        switch (iconName) {
            ...
            case 'ShieldAlert': return <svg {...props}><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>;
